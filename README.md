...
- **AI fallback** — for anything the rules can't identify, sends the process name + hostname + port to GPT-4o and asks it to classify. Results are cached to a local file so the same unknown host is only classified once.

### 4. Render
Displays everything as a live-updating table in the terminal using Bubbletea — a Go library for building terminal UIs. Columns are aligned, headers are styled, new connections are highlighted in green briefly.
...
| `classifier` | Port rules + hostname patterns + GPT-4o for unknowns |
| `graph` | Holds the current connection state, diffs new vs old |
...
**lsof** — pre-installed on every Mac, no special permissions needed for most connections, gives exactly what we need: process + network info together.
...
**Reverse DNS** — faster and more reliable than doing IP geolocation lookups. Most cloud providers have meaningful reverse DNS records (`ec2-52-x-x-x.compute-1.amazonaws.com` tells you it's AWS immediately).

**Rule-based classifier first, AI second** — rules are instant and free. AI is slow (100-500ms) and costs money. The vast majority of connections hit the rules. AI only fires for genuinely unknown hosts, and results are cached permanently so each unknown host is only classified once ever.
...
The project is built in phases, each one producing something runnable:
...
4. **AI layer** — add GPT-4o classification for unknowns. The intelligence layer.
...
A single binary you can run anywhere on your Mac. No config, no setup, no daemon. Just:

```
cartographer
```

And you see your machine's entire network activity, live, with context.
